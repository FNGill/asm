...
;   Perform this action:
;     RAX = RBX + RCX + 0x42
;   PRESERVE: All registers other than RAX
;;;;;;;;;;;;; YOUR CODE BELOW
	lea rax, [rbx]
	add rax, rcx
	add rax, 0x42 
...
;  -exchanges the values in regist`ers of op1 and op2
; GOAL:
;   Exchange values in registers RAX and RBX
...
;;;;;;;;;;;;; YOUR CODE BELOW
	...
	cmp rax, 42
	je .else1 ;if rax == 42, jump otherwise fall through to else
	mov rbx, 2
...
;;;;;;;;;;;;; YOUR CODE BELOW
	...
	mov rcx, 65 ; set counter to 0
	mov r8, 0
	.loop64:
		mov byte [rax + r8], r8b 
		dec rcx
		inc r8
		test rcx, rcx ; see if rcx is 0
		jnz .loop64 
;;;;;;;;;;;;; YOUR CODE ABOVE
	ret
...
;   PRESERVE: standard registers  my_strncmp("","",0) == 0
;         my_strncmp("","",1) == 0
;         my_strncmp("X","",1) == 1
;         my_strncmp("X","Y",1) == 1
;         my_strncmp("X","Y",0) == 0
